<?php

/*
Log Template
Neue Raidho Website					:	10%
*/

include "templates/header.php";
?>

<section id="loader" class="wrap">
	<div class="filter_header">
		<h2 class="Decima">Recent Activity</h2>
		<ul class="log_filter">
			<span class="Decima">Filter: </span>
			<li><a href="#">
				<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 16 16" enable-background="new 0 0 16 16" xml:space="preserve">
				<path  d="M12.667,12.424l0.325-0.116v-1.287h1.727l-0.002-3.451c0-1.719-0.686-3.33-1.932-4.536
					c-1.232-1.19-2.865-1.848-4.595-1.848c0,0-0.514-0.026-0.581-0.04c-0.295-0.572-0.888-0.97-1.578-0.97H5.542v0.896L4.962,1.038
					l-3.68,5.52V8.92h3.076V6.917h0.665c0.225,0.39,0.639,0.652,1.118,0.652c0.694,0,1.255-0.55,1.288-1.235h1.624l0.003,0.662
					l-4.809,3.396v1.916l0.325,0.116c0.322,0.112,0.539,0.418,0.539,0.754c0,0.339-0.217,0.644-0.539,0.757l-0.325,0.116v1.773h8.743
					v-1.773l-0.325-0.116c-0.322-0.113-0.539-0.418-0.539-0.757C12.127,12.842,12.345,12.536,12.667,12.424z"/>
				</svg>

			</a></li>
			<li><a href="#">
				<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
					  viewBox="0 0 16 16" enable-background="new 0 0 16 16" xml:space="preserve">
				<g>
					<path  d="M8,6.958c-1.57,0-2.85,1.278-2.85,2.85c0,1.57,1.279,2.847,2.85,2.847c1.571,0,2.85-1.276,2.85-2.847
						C10.849,8.236,9.571,6.958,8,6.958z"/>
					<path  d="M13.419,2.069h-1.357c-0.233-1.031-1.155-1.807-2.255-1.807H8c-1.103,0-2.021,0.775-2.256,1.807H2.579
						c-1.276,0-2.316,1.04-2.316,2.316v9.033c0,1.279,1.04,2.319,2.316,2.319h10.841c1.279,0,2.318-1.04,2.318-2.319V4.385
						C15.738,3.109,14.699,2.069,13.419,2.069z M8,2.069h1.808v1.021H8V2.069z M2.185,5.436c0-0.501,0.401-0.905,0.901-0.905
						c0.498,0,0.902,0.404,0.902,0.905c0,0.498-0.404,0.902-0.902,0.902C2.586,6.338,2.185,5.934,2.185,5.436z M8,14.183
						c-2.415,0-4.379-1.961-4.379-4.376S5.584,5.428,8,5.428s4.379,1.964,4.379,4.379S10.415,14.183,8,14.183z"/>
				</g>
				</svg>
			</a></li>
			<li><a href="#">
				<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
					  viewBox="0 0 16 16" enable-background="new 0 0 16 16" xml:space="preserve">
				<g>
					<path  d="M8,0.158C3.668,0.158,0.157,3.669,0.157,8c0,4.332,3.512,7.842,7.843,7.842s7.845-3.51,7.845-7.842
						C15.844,3.669,12.331,0.158,8,0.158z M14.019,8.636H8.577v1.782c1.422,0.105,2.77,0.604,3.926,1.459
						c0.28,0.209,0.34,0.606,0.132,0.89c-0.209,0.28-0.606,0.344-0.89,0.132c-0.936-0.689-2.021-1.099-3.168-1.202v2.108
						c0,0.353-0.286,0.636-0.635,0.636c-0.354,0-0.638-0.283-0.638-0.636v-2.097c-1.106,0.12-2.155,0.526-3.06,1.196
						c-0.114,0.083-0.247,0.123-0.378,0.123c-0.195,0-0.388-0.089-0.513-0.258c-0.209-0.283-0.149-0.681,0.133-0.89
						c1.127-0.833,2.435-1.328,3.817-1.451V8.636H1.865C1.512,8.636,1.228,8.353,1.228,8c0-0.352,0.284-0.635,0.637-0.635h5.439V5.589
						C5.913,5.463,4.597,4.962,3.465,4.118c-0.282-0.209-0.341-0.606-0.13-0.89c0.21-0.28,0.609-0.341,0.891-0.129
						c0.91,0.678,1.963,1.084,3.078,1.207V2.195c0-0.352,0.283-0.635,0.638-0.635c0.349,0,0.635,0.283,0.635,0.635v2.123
						c1.139-0.104,2.221-0.507,3.153-1.19c0.28-0.209,0.681-0.146,0.887,0.138c0.209,0.283,0.148,0.681-0.138,0.887
						C11.33,4.999,9.991,5.491,8.577,5.597v1.769h5.442c0.349,0,0.635,0.283,0.635,0.635C14.654,8.353,14.368,8.636,14.019,8.636z"/>
				</g>
				</svg>
			</a></li>
			<li><a href="#">
				<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
					  viewBox="0 0 16 16" enable-background="new 0 0 16 16" xml:space="preserve">
				<g>
					<polygon points="5.754,11.852 12.907,11.852 12.907,10.504 8.603,10.504 5.754,10.845 	"/>
					<polygon points="7.279,9.524 7.279,7.448 2.11,2.279 0.036,4.355 5.205,9.524 	"/>
					<polygon points="13.186,0.97 2.698,0.97 4.043,2.315 12.622,2.315 12.622,4.021 12.907,4.307 14.618,4.307
						14.618,13.684 4.043,13.684 4.043,10.233 2.698,8.888 2.698,15.03 15.964,15.03 15.964,3.748 	"/>
					<rect x="8.603" y="7.724" width="4.305" height="1.345"/>
					<polygon points="12.907,4.944 7.344,4.944 7.344,5.643 7.988,6.289 12.907,6.289 	"/>
				</g>
				</svg>
			</a></li>
			<li><a href="#">
				<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
					  viewBox="0 0 16 16" enable-background="new 0 0 16 16" xml:space="preserve">
				<path d="M14.224,5.099l1.457-1.547h-1.457c-0.354-1.97-2.079-3.466-4.147-3.466c-2.323,0-4.214,1.891-4.214,4.217
					c0,0.613,0.132,1.195,0.368,1.719C4.554,5.797,3.004,5.043,1.789,3.828C1.755,3.794,1.727,3.76,1.693,3.726
					C1.665,3.69,1.631,3.654,1.598,3.617L1.016,2.995l-0.29,0.802c-0.906,2.538-0.268,5.401,1.632,7.301
					c0.017,0.014,0.033,0.028,0.048,0.045c-0.211,0.017-0.425,0.031-0.645,0.031c-0.045,0-0.093-0.003-0.141-0.006l-0.996-0.039
					l0.366,0.773c1.153,2.437,3.632,4.012,6.315,4.012c3.852,0,6.988-3.137,6.988-6.991c0-0.687-0.104-1.358-0.296-1.997l1.683-1.156
					L14.224,5.099z"/>
				</svg>
			</a></li>
			<li><a href="#">
				<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
					  viewBox="0 0 16 16" enable-background="new 0 0 16 16" xml:space="preserve">
				<path  d="M8,4.799c1.016,0,1.844-0.826,1.844-1.844S9.016,1.11,8,1.11c-1.015,0-1.844,0.828-1.844,1.846
					S6.985,4.799,8,4.799z M8,2.441c0.282,0,0.513,0.229,0.513,0.515c0,0.282-0.23,0.513-0.513,0.513s-0.513-0.23-0.513-0.513
					C7.487,2.67,7.718,2.441,8,2.441z M9.92,6.561H6.061l-3.907,6.246v0.709h1.126l0.397-0.63l3.657,1.829v1.161h1.332v-1.161
					l3.658-1.829l0.393,0.63h1.13v-0.704L9.92,6.561z M7.334,13.226l-2.949-1.475l0.79-1.262l2.159,1.079V13.226z M7.334,10.081
					l-1.45-0.726l0.914-1.463h0.536V10.081z M8.666,7.892h0.517l0.924,1.47l-1.44,0.719V7.892z M8.666,13.226v-1.657l2.149-1.074
					l0.795,1.259L8.666,13.226z M12.592,2.956c0,1.039-0.34,2.022-0.967,2.83h-0.834l-0.053-0.053V4.734
					c0.341-0.526,0.522-1.137,0.522-1.778s-0.182-1.255-0.522-1.781V0.178l0.053-0.055h0.834C12.252,0.93,12.592,1.917,12.592,2.956z
					 M5.232,0.178v0.993c-0.34,0.529-0.521,1.14-0.521,1.781s0.182,1.255,0.521,1.781v0.999L5.18,5.786H4.352
					C3.725,4.978,3.38,3.992,3.38,2.953c0-1.036,0.34-2.022,0.967-2.829H5.18L5.232,0.178z"/>
				</svg>
			</a></li>
		</ul>
	</div>
	<ul class="log masonry">
		<!-- Masonry gutter sizer -->
		<li class="masonry_gutter"></li>
		<!-- Masonry columnWidth sizer -->
		<li class="masonry_column"></li>
		<!-- Masonry items -->
		<li class="dribble masonry_item">
			<a href="#"><img src="http://lorempixel.com/400/300/technics"></a>
			<p>Logo Nocturno</p>
		</li>
		<li class="instagram masonry_item">
			<a href="#"><img src="http://lorempixel.com/400/400/technics"></a>
			<p>Good Times; almost 2 years [Nov 2012] photo by @ Rotten</p>
		</li>
		<li class="share masonry_item">
			<a href="#"><img src="http://lorempixel.com/400/200/technics"></a>
			<p>Dâ Wearhouse’s new collection: Partners in Crime is now online</p>
		</li>
		<li class="twitter masonry_item hidden_element">
			<div class="red_bg white">
				<blockquote class="small_title">Charles Eames said: “Eventually everything connects – People, Ideas, Objects. The quality of the connections is the key to quality per se.”</blockquote>
			<p><a href="#">Twitted by Quiroga</a></p>
			</div>
		</li>
		<li class="work masonry_item hidden_element">
			<a href="#"><img src="http://lorempixel.com/400/400/technics"></a>
			<p class="small_title"><a href="#"><span class="red">New Project:</span> Explainer video for Colombian Tech Bootcamp Make it Real.</a></p>
		</li>
		<li class="blog masonry_item hidden_element">
			<a href="#"><img src="http://lorempixel.com/400/400/technics"></a>
			<p>Residente: El Potencial de Fundidora</p>
		</li>
		<li class="instagram masonry_item hidden_element">
			<a href="#"><img src="http://lorempixel.com/400/400/technics"></a>
			<p>#Linda #Dinosaurios #coder</p>
		</li>
		<li class="dribble masonry_item hidden_element">
			<a href="#"><img src="http://lorempixel.com/400/300/technics"></a>
			<p>New Homepage Quick Test</p>
		</li>
		<li class="share masonry_item hidden_element">
			<a href="#"><img src="http://lorempixel.com/400/300/technics"></a>
			<p>Rides for cash: ¿When will my bike pay for itself?</p>
		</li>
	</ul>
</section>

<?php

/*
Extended Nav + Footer
*/
include "templates/extended_nav.php";
include "templates/footer.php";

?>
